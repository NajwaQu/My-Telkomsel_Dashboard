...
# ══════════════════════════════════════════════
    ...
    # Macro average (rata-rata sederhana kelas Positif & Negatif)
    precision_macro = (precision + precision_neg) / 2
    recall_macro    = (recall + recall_neg) / 2
    f1_macro        = (f1 + f1_neg) / 2

    m1, m2, m3, m4 = st.columns(4)
    for col, label, val, color in [
        (m1, "Accuracy",  f"{accuracy*100:.2f}%",        "#6c8eff"),
        (m2, "Precision", f"{precision_macro*100:.2f}%", "#ff3b3b"),
        (m3, "Recall",    f"{recall_macro*100:.2f}%",    "#f5a623"),
        (m4, "F1-Score",  f"{f1_macro*100:.2f}%",        "#c47aff"),
    ]:
        with col:
            st.markdown(f"""
            <div class="metric-card" style="--accent:{color}">
            ...
            """, unsafe_allow_html=True)
    st.markdown('<div style="text-align:center; font-size:0.7rem; color:#8a6060; margin-top:4px;">Precision, Recall, dan F1-Score di atas adalah <strong>macro average</strong> dari kedua kelas</div>', unsafe_allow_html=True)
